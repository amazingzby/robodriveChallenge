...
def focal_loss(bce_loss):
    pt = torch.exp(-bce_loss)
    F_loss = focal_alpha * (1-pt)**focal_gamma * bce_loss
    return F_loss
    ...
    for i in range(1, n_classes):
        ...
        if torch.sum(completion_target) > 0:
            ...
            if torch.sum(p) > 0:
                ...
                #loss_precision = focal_loss(loss_precision)#focal Loss 1
                loss_class += loss_precision
            if torch.sum(completion_target) > 0:
                recall = nominator / (torch.sum(completion_target))
                loss_recall = F.binary_cross_entropy(recall, torch.ones_like(recall))
                #loss_recall = focal_loss(loss_recall) #focal loss2
            ...
            if torch.sum(1 - completion_target) > 0:
                ...
                #loss_specificity = focal_loss(loss_specificity) #focal Loss 3
