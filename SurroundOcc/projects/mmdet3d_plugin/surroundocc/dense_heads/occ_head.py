...
from projects.mmdet3d_plugin.surroundocc.loss.focal_loss import CustomFocalLoss#,CE_ssc_loss
from projects.mmdet3d_plugin.surroundocc.loss.lovasz_softmax import lovasz_softmax
...
#18类，原始版本为0 'noise',1-16 ,17free
#0与17对调
nusc_class_frequencies = np.array([
1689094172,#free
#1202967, #noise,忽略
499256, 30190, 474913, 4078450, 327017, 40273, 579250, 104073, 599519, 1375497, 25841748, 734070, 8296492, 11371339, 27194524, 28476243
])

#nusc_class_frequencies = np.array([
#    16890941722, #free
#    #120296750,#free
#    4992563, 301900, 4749134, 40784501, 3270178, 402736, 5792502, 1040733, 5995197,
#    13754976, 258417482, 7340707, 82964921, 113713395, 271945241, 284762437])/10.0
...
class OccHead(nn.Module): 
    def __init__(self,
        ...
        self.FocalLoss = CustomFocalLoss()
        self.class_weights = torch.from_numpy(1 / np.log(nusc_class_frequencies + 0.001))
        #class_weights 
        #[0.04706445 0.07621443 0.09694369 0.0765059  0.06569772 0.078754
        # 0.09430905 0.07536085 0.08655874 0.07516603 0.07074975 0.05859088
        # 0.0740392  0.06276935 0.06155131 0.05841624 0.05825951]
        #1 / mean(class_weights) = 13.9
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        #修改维度后测试输出shape
        #torch.Size([1, 256, 50, 50, 4])
        #torch.Size([1, 128, 100, 100, 8])
        #torch.Size([1, 64, 200, 200, 16])
        for i in range(len(self.deblocks)):
            ...
            elif i < 4: #增加H,W维度后对应修改，原方案不能pop最后一层
    ...
    def loss(self,
        ...
        if not self.use_semantic:
            ...
            #criterion = nn.CrossEntropyLoss(weight=self.class_weights.type_as(pred),
            ...
            for i in range(len(preds_dicts['occ_preds'])):
                if i < len(preds_dicts['occ_preds']) - 1:
                    continue
                #i = len(preds_dicts['occ_preds']) - 1
                ...
                #loss_occ_1 = criterion(pred, gt.long())
                #loss_occ_2 = sem_scal_loss(pred, gt.long())
                #loss_occ_3 = geo_scal_loss(pred, gt.long())
                #criterion = nn.CrossEntropyLoss(weight=self.class_weights.type_as(pred),ignore_index=255, reduction="mean")
                #criterion = nn.CrossEntropyLoss(ignore_index=255, reduction="mean")
                
                
                #case1
                #layerScale = (0.5)**(len(preds_dicts['occ_preds']) - 1 -i)
                #if i < len(preds_dicts['occ_preds']) - 1:
                #    loss_dict['loss_ce_{}'.format(i)] = criterion(pred, gt.long()) * layerScale
                #else:
                #    loss_dict['loss_focal_loss_{}'.format(i)] = self.FocalLoss(pred, gt.long(),self.class_weights.type_as(pred),ignore_index=255) * layerScale
                #loss_dict['loss_ce_{}'.format(i)] = criterion(pred, gt.long()) * layerScale
                #loss_dict['loss_focal_loss_{}'.format(i)] = self.FocalLoss(pred, gt.long(),self.class_weights.type_as(pred),ignore_index=255)# * layerScale
                loss_dict['loss_ce_{}'.format(i)] = criterion(pred, gt.long())
                loss_dict['loss_sem_{}'.format(i)] = sem_scal_loss(pred, gt.long())# * layerScale
                loss_dict['loss_geo_{}'.format(i)] = geo_scal_loss(pred, gt.long())# * layerScale
                #loss_dict['loss_lovasz_{}'.format(i)] = lovasz_softmax(torch.softmax(pred, dim=1), gt.long(), ignore=255) * layerScale
                
                #loss1 = self.FocalLoss(pred, gt.long(),self.class_weights.type_as(pred),ignore_index=255)
                #loss1 = CE_ssc_loss(pred, gt.long(),self.class_weights.type_as(pred),ignore_index=255)
                #                                    )
                #loss1 = criterion(pred, gt.long())
                #loss1 = self.FocalLoss(pred, gt.long(),self.class_weights.type_as(pred),ignore_index=255)
                #loss2 = sem_scal_loss(pred, gt.long())
                #loss3 = geo_scal_loss(pred, gt.long())
                #loss4 = lovasz_softmax(torch.softmax(pred, dim=1), gt.long(), ignore=255)
                #loss_occ_i = loss1 + loss2 + loss3 + loss4
